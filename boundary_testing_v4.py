...
def menu_call(): #Definition created as the menu.
    ...
    if menu_option == 2: #If a users types in 2 as their answer, the menu would call the tickets definition.
        ...
        time.sleep(2) #Using the import time function, I could pause the programe for 4 seconds.
...
def tickets(): #Definition created for purchasing tickets.
    ...
    print("There are {} child catagories.".format(ticket_catagories.count('CH')))
...
def ticket_win():
    ...
    while guess >= 1:
         try:
            num_guess = float(input("How many students approximitly are at Henderson High School? ")).STRIP()
            if num_guess == 1000:
...
def payment():
   ...
   for payment_input in range(3):
       payment_input = str(input("To verifiy your payment option, please type the payment option now? ")).strip().lower()
       if payment_input in payment_types:
            print("We accept your payment method of {}.".format(payment_input.capitalize()))
            if payment_input ==  "cash":
                print("Please pay when you arrive at the production.")
                print("When you arrive, please indicate your order number.")
               
            if payment_input == "bank transfer":
                print("Option 1")
                
            if payment_input == "debit card":
                print("Option 2")
                
            else:
                print("Sorry, your payment option is not accepted here, please try another payment option.")
                
       else:
            print("We don't accept your payment method.")


   print("Sorry, you have entered an incoreect payment 3 times, good bye!")
   quit()
