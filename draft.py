...
pizza_options = {"Pizza 1":'Australlia', "Pizza 2":'France', "Pizza 3":'Korea', "Pizza 4":'India', "Pizza 5":'Fiji', "Pizza 6":'Germany', "Pizza 7":'Wellington', "Pizza 8":'Southern Italy', "Pizza 9":'New York', "Pizza 10":'London', "Pizza 11":'Rome', "Pizza 12":'Chicago', } # Dictonary for all the pizza options in the new order function. 
...
def service_needed(): # Defining the funtion of services_needed when called.
    global user_option_0 # Making the user_option_0 variable global to apply to all functions in this script.
    global user_option_2 # Making the user_option_2 variable global to apply to all functions in this script.
    ...
    user_option_2 = tk.Button(window, text =("Delivery"), bg=button_background, fg=button_text, font=font_global, width=20, command=delivery) # Creating a button which will take the user to the delivery function/new order function.
    user_option_2.pack() # Putting the user_option_2 button onto the GUI platform.
    user_option_2.place(relx=0.53, rely=0.8) # Positioning the user_option_2 button to a designated x and y positions according to the size of the window.

    user_option_0 = tk.Button(window, text =("Pick-Up"), bg=button_background, fg=button_text, font=font_global, width=20, command=pick_up) # Creating a button which will take the user to the pick-up function/new order function.
    user_option_0.pack() # Putting the user_option_0 button onto the GUI platform.
    user_option_0.place(relx=0.33, rely=0.8) # Positioning the user_option_0 button to a designated x and y positions according to the size of the window.
...
def pick_up(): # Defining the funtion of pizza_Palace when called.
    ...
    user_option_0.place_forget() # Forget the object/Delete the object from the window.
    user_option_2.place_forget() # Forget the object/Delete the object from the window. 
...
def new_order(): # Defining the funtion of new_order when called.
        ...
        canva.config(width=0, height=0) # Set the canvas size to (0X0).
...
def payment(): # Defining the funtion of payment when called.
    ...
    display_customer_name.place(relx=0.23, rely=0.3) # Positioning the display_customer_name label to a designated x and y positions according to the size of the window.
    ...
    display_customer_phone_number.place(relx=0.3, rely=0.4) # Positioning the display_customer_phone_number label to a designated x and y positions according to the size of the window.
...
def restart(): # Defining the funtion of restart when called.
    window.destroy() # Asking the python window to be destroyed.
    os.system("python draft.py") # Asking the operating system (os) to reopen the draft python script on the computer.
    sys.exit() # Asking the python system to exit this python programe.
